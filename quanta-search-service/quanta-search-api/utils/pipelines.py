...
async def load_total_doc_count_pipeline(uid:str):
    return [
            {
                "$match": {"metadata.uid": uid}
            },
            {
                "$project": {"metadata":1, "_id":1}
            },
            { 
                "$group": { 
                "_id": None,
                "uniqueFileNames": { "$addToSet": "$metadata.file_name" }
                }
            },
            { 
                "$project": { 
                "_id": 0,
                "uniqueFileNames": { "$size": "$uniqueFileNames" }
                }
            }
        ]
...
async def get_list_files_pipeline(filters:dict):
    return [
            {"$match": filters},
            {
                "$project": {
